...
const Home = ({ token: appToken, setToken: setAppToken }) => {
  ...
  const tokenValue = typeof appToken !== "undefined" ? appToken : null;

  const syncAuthState = () => {
    try {
      const storedToken = (localStorage.getItem("token") || tokenValue || "").trim();
  ...
  const applyAnalysisResult = (data, analyzedText = "") => {
    ...
    const level = normalizeRiskLevel(data?.riskLevel || "Low", confidenceValue);
  ...
  const startRecording = async () => {
    try {
      ...
      mediaRecorderRef.current.onstop = async () => {
        ...
        // auto-play the recorded audio once
        try {
          const player = new Audio(preview);
  ...
  const handleSaveBookmark = async () => {
    ...
    if (!authToken) {
      handleAuthFailure("Please log in to save bookmarks.");
      return;
    }

    if (!hasAnalyzed) {
      setBookmarkStatus("Run analysis first before saving.");
      return;
    }
    ...
    try {
      ...
      const urlValue = /^https?:\/\//i.test(inputValue.trim())
        ? inputValue.trim()
        : "";
      ...
      const payload = {
        url: urlValue,
        title:
          (inputValue.trim() || extractedText || "").slice(0, 100) ||
          "Saved Fake News Check",
        content: extractedText || inputValue.trim() || "",
        inputType: urlValue ? "url" : "text",
        result: resultLabel,
        confidence,
        sourceLinks: sourceLinks.map((item) => item.url),
        claimsExtracted: claimCount,
      };

      const saved = await bookmarksApi.create(payload);

      setBookmarkSaved(true);
      setBookmarkStatus("Report saved to bookmarks.");
      window.dispatchEvent(new Event("bookmark-updated"));
    } catch (error) {
      setBookmarkStatus(error.message || "Failed to save bookmark.");
    } finally {
      ...
      <section className="result-section">
        <div className="result-left">
          ...
          <div className="result-actions">
            ...
            <button
              type="button"
              className={bookmarkSaved ? "saved-btn" : "save-btn"}
              disabled={!hasAnalyzed || bookmarkSaved || bookmarkLoading}
              onClick={handleSaveBookmark}
            >
              <Bookmark size={16} />
              {bookmarkSaved ? "Saved" : bookmarkLoading ? "Saving..." : "Save Report"}
            </button>
          ...
          {bookmarkStatus && (
            <div className="settings-success" style={{ marginTop: "0.8rem" }}>
              {bookmarkStatus}
            </div>
          )}
        </div>
